...
func TestCleanSubtitleCuesPrefersHigherConfidenceOverlappingChunkDuplicate(t *testing.T) {
	...
	firstOrigin := Origin{Index: 0, Start: 0, End: 20 * time.Second}
	secondOrigin := Origin{Index: 1, Start: 19800 * time.Millisecond, End: 40 * time.Second}
	cues := []Cue{
		{
			Start: 19800 * time.Millisecond, End: 20 * time.Second, Text: "같은 발화", Probability: 0.6, Origin: firstOrigin,
			Tokens: []Token{{Start: 19800 * time.Millisecond, End: 20 * time.Second, Text: " 같은 발화", Probability: 0.6, Origin: firstOrigin}},
		},
		{
			Start: 19800 * time.Millisecond, End: 20 * time.Second, Text: "같은 발화", Probability: 0.9, Origin: secondOrigin,
			Tokens: []Token{{Start: 19800 * time.Millisecond, End: 20 * time.Second, Text: " 같은 발화", Probability: 0.9, Origin: secondOrigin}},
		},
	}

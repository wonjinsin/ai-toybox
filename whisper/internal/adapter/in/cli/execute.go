...
type Factory func(portin.ProgressObserver) portin.BatchRunner

func Execute(ctx context.Context, args []string, stdout, stderr io.Writer, factory Factory) int {
	request, err := ParseOptions(args)
	if err != nil {
	...
	progress := NewProgressObserver(stderr)
	defer progress.Close()
	batch, err := factory(progress.Observe).Run(ctx, request)
	progress.Close()
	if err != nil {
	...
	for _, result := range batch.Results {
		if port.IsOutputExistsError(result.Err) {
			skipped++
	...
	if batch.Directory {
		fmt.Fprintf(stdout, "요약: 성공 %d개, 건너뜀 %d개, 실패 %d개\n", len(batch.Results)-skipped-failures, skipped, failures)
	}
	if failures > 0 {
		return 1
	}

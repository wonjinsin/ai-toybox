...
func TestIncrementalCheckpointJSONPreservesCueDetails(t *testing.T) {
	...
	checkpoint := newIncrementalCheckpoint(90*time.Second, []domain.Chunk{{Start: 10 * time.Second, End: 25 * time.Second}}, 1, []domain.Cue{{
...
func TestLoadIncrementalCheckpointRejectsDifferentFingerprint(t *testing.T) {
	...
	checkpoint := newIncrementalCheckpointWithFingerprint(fingerprint, 60*time.Second, []domain.Chunk{{Start: time.Second, End: 2 * time.Second}}, 1, nil)
...
func TestLoadIncrementalCheckpointRejectsVersionBeforeQuietSpeechNormalization(t *testing.T) {
	...
	checkpoint := newIncrementalCheckpointWithFingerprint(legacyFingerprint, 60*time.Second, []domain.Chunk{{Start: time.Second, End: 2 * time.Second}}, 1, nil)
	if err := persistIncrementalCheckpoint(path, checkpoint); err != nil {
		t.Fatal(err)
	}
...
func TestLoadIncrementalCheckpointRejectsRetryCursorDuringTranscription(t *testing.T) {
	...
	path := filepath.Join(directory, ".input.srt.whisper-local-checkpoint.json")
	fingerprint := incrementalFingerprint{PipelineVersion: incrementalPipelineVersion, Language: "ja"}
	checkpoint := newIncrementalCheckpointWithState(fingerprint, portout.StageTranscribing, 1, 60*time.Second, []domain.Chunk{{Start: time.Second, End: 2 * time.Second}}, 1, []domain.Cue{{
...
func newIncrementalCheckpoint(mediaDuration time.Duration, chunks []domain.Chunk, completedChunks int, cues []domain.Cue) incrementalCheckpoint {
	return newIncrementalCheckpointWithFingerprint(incrementalFingerprint{}, mediaDuration, chunks, completedChunks, cues)
}

func newIncrementalCheckpointWithFingerprint(fingerprint incrementalFingerprint, mediaDuration time.Duration, chunks []domain.Chunk, completedChunks int, cues []domain.Cue) incrementalCheckpoint {
	return newIncrementalCheckpointWithState(fingerprint, portout.StageTranscribing, 0, mediaDuration, chunks, completedChunks, cues)
}

...
func TestWhisperCommandRunnerRetriesOnceAfterGPUOutOfMemory(t *testing.T) {
	...
	runner := NewCommandRunner(2, func(context.Context, string, []string) (string, error) {
		if calls.Add(1) == 1 {
...
func TestWhisperCommandRunnerWaitsForActiveCommandBeforeOOMRetry(t *testing.T) {
	...
	runner := NewCommandRunner(2, func(_ context.Context, executable string, _ []string) (string, error) {
		switch executable {
		case "hold":
			close(holdStarted)
			<-holdRelease

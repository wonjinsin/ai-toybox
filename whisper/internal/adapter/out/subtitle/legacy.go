...
func PostProcess(content string, segments []domain.SpeechSegment) string {
	processed := collapseRepeatedCues(content)
	processed = alignCueStartsToSpeech(processed, segments)
	return clampCueDurations(processed)
}

func alignCueStartsToSpeech(content string, vadSegments []domain.SpeechSegment) string {
	...
	for index, line := range lines {
		...
		overlappingSegments := make([]domain.SpeechSegment, 0, 1)

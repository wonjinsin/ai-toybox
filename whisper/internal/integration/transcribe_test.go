...
func TestTranscribePersistsPartialAfterCompletedBatchWhenNextBatchFails(t *testing.T) {
	...
	var mainCalls int
	runner := whisper.NewCommandRunner(1, func(_ context.Context, executable string, args []string) (string, error) {
	...
	_, err := transcribeWithRunner(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "srt",
		ModelPath: modelPath,
	...
	if err == nil || !strings.Contains(err.Error(), "second batch failed") {
		t.Fatalf("transcribeWithRunner() error = %v, want second batch failure", err)
	}
	if mainCalls != 2 {
...
func TestTranscribeReconcilesBoundaryAcrossTranscriptionBatches(t *testing.T) {
	...
	outputPath, err := transcribeWithRunner(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "srt",
		ModelPath: modelPath,
	...
	if err != nil {
		t.Fatalf("transcribeWithRunner() error = %v", err)
	}
	if mainCalls != 2 {
		t.Fatalf("main transcription calls = %d, want 2", mainCalls)
	}
	...
	partialPath, checkpointPath := artifactPaths(strings.TrimSuffix(outputPath, ".srt"))
...
func TestTranscribeResumesWithoutRetranscribingCheckpointedChunks(t *testing.T) {
	...
	options := portin.Request{InputPath: inputPath, Language: "ko", Format: "srt", ModelPath: modelPath}

	if _, err := transcribeWithRunner(context.Background(), options, nil, runner); err == nil {
	...
	outputPath, err := transcribeWithRunner(context.Background(), options, func(event portin.ProgressEvent) {
		if event.Kind == portin.EventResumed {
			progressMessages = append(progressMessages, fmt.Sprintf("체크포인트 재개: %d/%d개", event.Completed, event.Total))
		}
	}, runner)
	if err != nil {
...
func TestTranscribeResumesAfterCompletedLowConfidenceRetryBatch(t *testing.T) {
	...
	if _, err := transcribeWithRunner(context.Background(), options, nil, runner); err == nil || !strings.Contains(err.Error(), "second retry batch failed") {
	...
	outputPath, err := transcribeWithRunner(context.Background(), options, nil, runner)
	if err != nil {
		t.Fatalf("resumed transcription error = %v", err)
	}
	if vadCalls != 1 {
...
func TestTranscribePreservesRetryBatchCursorWhenResumedRetryFailsAgain(t *testing.T) {
	...
	if _, err := transcribeWithRunner(context.Background(), options, nil, runner); err == nil || !strings.Contains(err.Error(), "first resumed cue failed") {
		t.Fatalf("first transcription error = %v, want first retry failure", err)
	}
	if _, err := transcribeWithRunner(context.Background(), options, nil, runner); err == nil || !strings.Contains(err.Error(), "second resumed cue failed") {
	...
	if checkpoint.Stage != portout.StageRetrying || checkpoint.RetryCursor != 128 {
...
func TestTranscribeKeepsIncrementalArtifactsWhenFinalOutputAppearsDuringRun(t *testing.T) {
	...
	if !port.IsOutputExistsError(err) {
	...
	partialPath, checkpointPath := artifactPaths(strings.TrimSuffix(finalPath, ".srt"))
...
`)
	...
	outputPath, err := transcribe(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "txt",
		ModelPath: modelPath,
...
`)
	...
	inputPath, modelPath := writeInputAndModel(t, tempDir)
	if _, err := transcribe(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "txt",
		ModelPath: modelPath,
...
func TestTranscribeRequiresVADModel(t *testing.T) {
	...
	_, err := transcribe(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "txt",
		ModelPath: modelPath,
...
`)
	...
	resultPath, err := transcribe(context.Background(), portin.Request{
		InputPath: inputPath,
		Language:  "ko",
		Format:    "txt",
		ModelPath: modelPath,

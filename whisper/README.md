...
6. 저신뢰 구간은 최대 128개씩 beam size 8의 결정적 디코딩으로 재시도한 뒤, 신뢰도·반복되는 일본어 환각 상투문구·비언어 필터와 가독성 규칙을 적용해 TXT, SRT, VTT를 렌더링합니다. 일본어 또는 자동 언어 모드에서 `ご視聴ありがとうございました`, `おめでとうございます` 계열은 같은 파일에 두 번 이상 나올 때만 제거해 실제 단일 발화는 보존합니다.

...
`-format srt`는 음성 조각과 저신뢰 재시도 구간을 각각 최대 128개씩 전사합니다. 배치 하나가 끝날 때마다 최종 출력 옆의 `<파일명>.partial.srt`를 갱신하므로 처리 중에도 완료된 구간을 확인할 수 있습니다. 이 파일은 아직 뒤쪽 발화와 저신뢰 재시도 결과가 빠질 수 있는 임시 자막입니다.
...
- 발화 탐지: `whisper-vad-speech-segments`와 Silero VAD 필수. 15초마다 VAD 상태를 초기화해 앞 구간의 배경음 때문에 뒤의 작은 발화가 누락되는 현상을 줄이며, 도구나 VAD 모델이 없으면 명확한 오류로 중단
- 자막 시간: 최대 3초의 대화 중 멈춤은 한 조각으로 묶어 자동 언어 판정 문맥을 보존하고, 원본 타임라인 기준 최대 20초 조각으로 전사해 긴 무음 후에도 시간 밀림 방지
...
`-parallel`은 동시에 실행할 최대 파일 작업자 수입니다. 한 파일이 끝나거나 기존 결과 때문에 건너뛰면 빈 작업 슬롯에서 다음 파일을 바로 시작하며, 폴더의 모든 파일을 확인할 때까지 계속합니다. `-force`가 없을 때 대상 결과 파일이 이미 있으면 해당 파일은 실패가 아닌 건너뜀으로 집계하고 기존 결과를 보존합니다. `-force`를 사용하면 기존 결과도 다시 전사해 덮어씁니다.

마지막에는 `요약: 성공 N개, 건너뜀 N개, 실패 N개`를 출력합니다. 파일 하나가 실패해도 나머지는 처리하며, 실제 실패가 하나라도 있으면 종료 코드는 `1`입니다. 건너뜀만 있으면 정상 종료 코드 `0`입니다.

기본 병렬도는 `1`입니다. `-parallel`을 높이면 파일별 FFmpeg 전처리와 VAD는 여러 작업자에서 진행하지만, 약 1.5~3GB 모델을 각각 다시 올리지 않도록 `whisper-cli`는 한 번에 1개만 실행합니다. Metal GPU 메모리 부족이 발생하면 실패한 명령을 한 번만 자동 재시도합니다. 재시도도 실패하거나 OOM이 아닌 오류가 발생하면 상세 원인을 보존해 해당 파일을 실패로 처리합니다.
...
2. 정규화 WAV를 15초 단위로 나누고 전용 `whisper-vad-speech-segments`를 최대 4개씩 실행합니다. 조각마다 Silero VAD 상태를 새로 시작한 뒤 결과를 원본 절대 시간으로 복원해, 긴 파일의 앞 소리가 뒤 발화 판정에 누적되는 것을 막습니다.
3. 발화 사이 최대 3초의 자연스러운 멈춤을 문맥으로 보존하고, 경계 여유 150ms를 포함한 최대 20초 WAV 조각을 최대 4개씩 병렬 생성합니다.
4. 본 전사는 beam size 1을 사용합니다. SRT는 최대 128개 조각 단위로 `large-v3`를 실행하고 각 배치의 JSON과 partial SRT를 저장합니다. TXT와 VTT는 기존처럼 한 번의 다중 파일 실행으로 전체 JSON을 생성합니다.
5. 각 JSON 시간에 원본 조각 시작 시간을 더해 절대 시간을 복원합니다.
6. 저신뢰 구간은 최대 128개씩 beam size 8의 결정적 디코딩으로 재시도한 뒤, 신뢰도·반복되는 일본어 환각 상투문구·비언어 필터와 가독성 규칙을 적용해 TXT, SRT, VTT를 렌더링합니다. 일본어 또는 자동 언어 모드에서 독립 자막으로 반복되는 `ご視聴ありがとうございました`·`次の動画でお会いしましょう`·`おめでとうございます` 계열은 2회, `おやすみなさい` 계열은 3회부터 제거합니다. 정상 발화가 흔한 `ありがとうございました`와 `ありがとうございます`는 서로 합산하지 않고 같은 형태가 4회 이상일 때만 제거합니다. 문장 안에 포함되거나 한 번만 나온 문구는 보존합니다.
...
- Homebrew
- FFmpeg와 `whisper.cpp`(`whisper-cli`, `whisper-vad-speech-segments` 실행 파일 포함)
